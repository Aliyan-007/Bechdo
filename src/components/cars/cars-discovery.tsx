...
export function CarsDiscovery({
  ...
  const recentlyAddedParam = searchParams.get("recentlyAdded") === "true";
  const isLocallyAssembledParam = searchParams.get("isLocallyAssembled");
  ...
  const activeFilters = useMemo(() => {
    ...
    if (recentlyAddedParam) list.push({ key: "recentlyAdded", label: "New & Recently Added", value: "true" });
    if (isLocallyAssembledParam === "true") list.push({ key: "isLocallyAssembled", label: "Assembly: Local CKD", value: "true" });
    if (isLocallyAssembledParam === "false") list.push({ key: "isLocallyAssembled", label: "Assembly: Official CBU", value: "false" });
    if (qParam) list.push({ key: "q", label: `Search: "${qParam}"`, value: qParam });
    return list;
  }, [brandParam, bodyParam, fuelParam, transParam, budgetParam, favoritesParam, featuredParam, popularParam, recentlyAddedParam, isLocallyAssembledParam, qParam]);
  ...
  const filteredVehicles = useMemo(() => {
    ...
    if (recentlyAddedParam) {
      list = list.filter((v) => v.isRecentlyAdded || v.badge === "New");
    }

    if (isLocallyAssembledParam === "true") {
      list = list.filter((v) => v.isLocallyAssembled === true);
    } else if (isLocallyAssembledParam === "false") {
      list = list.filter((v) => v.isLocallyAssembled === false);
    }
    ...
    return list;
  }, [initialVehicles, qParam, brandParam, bodyParam, fuelParam, transParam, budgetParam, favoritesParam, featuredParam, popularParam, recentlyAddedParam, isLocallyAssembledParam, sortParam]);
