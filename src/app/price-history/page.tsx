...
export default async function PriceHistoryPage() {
  ...
  const points = priceHistories.map((ph: any) => {
  ...
  const validPrices = points.map((p: any) => p.value).filter((v: any) => v > 0);
  ...
  const currentPoints = points.filter((p: any) => p.year >= 2024);
  const historicalPoints = points.filter((p: any) => p.year < 2015);

  const avgCurrentPriceLakh =
    currentPoints.length > 0
      ? Math.round(
          (currentPoints.reduce((sum: number, p: any) => sum + p.value, 0) /
  ...
  const avgHistoricalPriceLakh =
    historicalPoints.length > 0
      ? Math.round(
          (historicalPoints.reduce((sum: number, p: any) => sum + p.value, 0) /
  ...
  const brandNames = allBrands.map((b: any) => b.name);
